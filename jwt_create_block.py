...
from nio.properties import PropertyHolder, StringProperty, Property, \
...
class JWTCreate(EnrichSignals, JWTBase):
    ...
    exp_minutes = Property(
        title='Valid For Minutes (blank for no exp claim)',
        order=3,
        allow_none=True)
    ...
    def process_signal(self, signal, input_id=None):
        ...
        try:
            ...
            _token = jwt.encode(
                _newclaims, _key, algorithm=_algorithm.value).decode('UTF-8')
